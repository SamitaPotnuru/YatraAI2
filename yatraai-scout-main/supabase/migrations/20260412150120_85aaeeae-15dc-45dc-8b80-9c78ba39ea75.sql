...
CREATE TABLE public.expenses (
  ...
  category TEXT NOT NULL CHECK (category IN ('stay', 'food', 'transport', 'activities', 'other')),
...
CREATE POLICY "Travel buddies are viewable by authenticated users" ON public.travel_buddies FOR SELECT TO authenticated USING (true);
CREATE POLICY "Users can create travel buddies" ON public.travel_buddies FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
